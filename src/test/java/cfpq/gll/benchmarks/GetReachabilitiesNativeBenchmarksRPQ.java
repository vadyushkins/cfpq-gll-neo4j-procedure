...
public class GetReachabilitiesNativeBenchmarksRPQ {
    ...
    public void runTest(
        ...
        final String reg1 = Files.readString(
            Paths.get("src", "test", "resources", "cfpq", "gll", "benchmarks", "grammar", "native", graph, "reg1.txt")
        ).replace("\n", "");
        final String reg2 = Files.readString(
            Paths.get("src", "test", "resources", "cfpq", "gll", "benchmarks", "grammar", "native", graph, "reg2.txt")
        ).replace("\n", "");
        final String reg3 = Files.readString(
            Paths.get("src", "test", "resources", "cfpq", "gll", "benchmarks", "grammar", "native", graph, "reg3.txt")
        ).replace("\n", "");
        final String reg4 = Files.readString(
            Paths.get("src", "test", "resources", "cfpq", "gll", "benchmarks", "grammar", "native", graph, "reg4.txt")
        ).replace("\n", "");
        ...
        try (Session session = driver.session()) {
            ...
            for (String grammarName : new String[]{"reg1", "reg2", "reg3", "reg4"}) {
                ...
                File resultsDir = Paths.get("src", "test", "resources", "cfpq", "gll", "benchmarks", "results", "native_rpq", graph).toFile();
                resultsDir.mkdirs();
                ...
                for (int chunkSize : new int[]{1, 10, 100}) {
                    ...
                    for (int chunkStart = 0; chunkStart < selectedNodesSize; chunkStart += chunkSize) {
                        ...
                        StringBuilder matchNodes = new StringBuilder();
                        for (int nodeId = chunkStart; nodeId < chunkEnd; ++nodeId) {
                            if (nodeId != chunkStart) {
                                matchNodes.append(",");
                            }
                            matchNodes.append(
                                String.format("%s", selectedNodes.get(nodeId))
                            );
                        }

                        String query =
                            String.format("MATCH (s:Node WHERE s.id IN [%s]) ", matchNodes) +
                                String.format("MATCH %s ", grammar) +
                                "RETURN s, t";
