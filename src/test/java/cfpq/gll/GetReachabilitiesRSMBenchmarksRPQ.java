...
public class GetReachabilitiesRSMBenchmarksRPQ {
    ...
    public void runTest(
        ...
        final String reg1 = Files.readString(
            Paths.get("src", "test", "resources", "grammar", "rsm", graph, "reg1.txt")
        ).replace("\n", ";");
        final String reg2 = Files.readString(
            Paths.get("src", "test", "resources", "grammar", "rsm", graph, "reg2.txt")
        ).replace("\n", ";");
        final String reg3 = Files.readString(
            Paths.get("src", "test", "resources", "grammar", "rsm", graph, "reg3.txt")
        ).replace("\n", ";");
        final String reg4 = Files.readString(
            Paths.get("src", "test", "resources", "grammar", "rsm", graph, "reg4.txt")
        ).replace("\n", ";");
        ...
        try (Session session = driver.session()) {
            ...
            for (String grammarName : new String[]{"reg1", "reg2", "reg3", "reg4"}) {
                ...
                File resultsDir = Paths.get("src", "test", "resources", "results", "rsm_rpq", graph).toFile();
                resultsDir.mkdirs();
